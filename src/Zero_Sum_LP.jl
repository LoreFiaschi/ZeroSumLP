...
function solve_patrolling(P::Vector{T}, D::AbstractMatrix{Bool}, τ::Integer;
	...
	A_normalization = zeros(T,n_nodes,n_nodes^2);
	for i=1:n_nodes
		A_normalization[i,(i-1)*n_nodes+1:i*n_nodes] .= 1;
	end
	
	b_normalization = ones(n_nodes,1);
	
	t_normalization = zeros(n_nodes);
	...
	for i=1:n_nodes
		A_density[i,[(j-1)*n_nodes+i for j=1:τ]] .= 1;
	end
	
	b_density = ones(T,n_nodes,1);
	
	t_density = zeros(n_nodes);
    ...
    t =  [t_payoff; t_traversing; t_normalization; t_reciprocity; t_longrun; t_density; t_complementarity];
	...
	A = [A_payoff                 zeros(T,n_nodes,n_nodes^2) zeros(T,n_nodes,τ*n_nodes);
		 ...
		 zeros(T,n_nodes,n_nodes) 	  A_normalization            zeros(T,n_nodes,τ*n_nodes);
	...
	b = [b_payoff; b_traversing; b_normalization; b_reciprocity; b_longrun; b_density; b_complementarity];
